 ...
 * @internal
 */
export const handleFormatContainerAsync: AsyncContentModelBlockHandler<ContentModelFormatContainer> = async (
    doc: Document,
    parent: Node,
    container: ContentModelFormatContainer,
    context: ModelToDomContext,
    ...
    if (element) {
        ...
        await context.asyncModelHandlers.blockGroupChildrenAsync(doc, element, container, context);
    } else if (!isBlockGroupEmpty(container)) {
        const containerNode = doc.createElement(container.tagName);
        ...
        if (container.tagName == 'pre') {
            stackFormat(context, PreChildFormat, async () => {
                await context.asyncModelHandlers.blockGroupChildrenAsync(
                    doc,
                    containerNode,
                    container,
                    context
