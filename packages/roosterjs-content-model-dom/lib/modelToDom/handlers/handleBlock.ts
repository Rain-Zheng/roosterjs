 ...
 * @internal
 */
export const handleBlockAsync: AsyncContentModelBlockHandler<ContentModelBlock> = async (
    doc: Document,
    parent: Node,
    block: ContentModelBlock,
    context: ModelToDomContext,
    ...
    const handlers = context.modelHandlers;
    const asyncHandlers = context.asyncModelHandlers;
    ...
    switch (block.blockType) {
        ...
        case 'BlockGroup':
            switch (block.blockGroupType) {
                case 'General':
                    refNode = await asyncHandlers.generalBlockAsync(
                        doc,
                        parent,
                        block,
                        context,
                ...
                case 'FormatContainer':
                    refNode = await asyncHandlers.formatContainerAsync(
                        doc,
                        parent,
                        block,
                        context,
                ...
                case 'ListItem':
                    refNode = await asyncHandlers.listItemAsync(
                        doc,
                        parent,
                        block,
                        context,
