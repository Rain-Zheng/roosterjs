 ...
 * @internal
 */
export const handleListItemAsync: AsyncContentModelBlockHandler<ContentModelListItem> = async (
    doc: Document,
    parent: Node,
    listItem: ContentModelListItem,
    context: ModelToDomContext,
    ...
    if (level) {
        ...
        stackFormat(context, listItem.formatHolder.format, async () => {
            await context.asyncModelHandlers.blockGroupChildrenAsync(doc, li, listItem, context);
        });
    } else {
        // There is no level for this list item, that means it should be moved out of the list
        // For each paragraph, make it not implicit so it will have a DIV around it, to avoid more paragraphs connected together
